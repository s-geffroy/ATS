...
- **`lighthouse/ci-assert.mjs` runner multi-run avec médiane par catégorie** : ajout des env `LIGHTHOUSE_RUNS` (défaut 1 — rétrocompatible) et `LIGHTHOUSE_MIN_SCORE` (défaut 0.90). Pour chaque (page, form) le runner exécute N runs, persiste chacun sous `<slug>-run<N>.json`, calcule la **médiane par catégorie**, et gate sur la médiane. Le run dont la performance équivaut à la médiane est copié sous `<slug>.json` (artefact canonique). Tableau de résumé enrichi : ligne médiane + ligne « min des N runs » comme plancher de bruit visible. Justification : Lighthouse a une variance ~20 points observée sur des pages mono-fichier (FAQ desktop a oscillé 71→94 sans changement de code) ; la médiane de 3 absorbe les outliers cold-cache + GC sans baisser le seuil.
- **`.github/workflows/lighthouse.yml` job `lighthouse` localhost (médiane de 3) + nouveau job `lighthouse-prod`** : le job localhost passe à `LIGHTHOUSE_RUNS=3` (timeout-minutes 10 → 30 pour absorber 4 pages × 2 form × 3 runs ≈ 18 min worst case). **Nouveau job `lighthouse-prod`** mesure contre `https://s-geffroy.github.io/ATS` à seuil **≥ 95** (gzip + CDN améliorent typiquement 5-15 pts vs localhost). Le job prod est `continue-on-error: true` (un hoquet GitHub Pages ne doit pas bloquer un merge) et skip sur les PR (le site déployé reflète `main`, pas la branche PR). Le job localhost reste la gate canonique. Artefacts uploadés séparément (`lighthouse-reports-localhost` vs `lighthouse-reports-prod`).
- **`lighthouse/README.md` — section « Role in the project » étendue + nouveau bloc « Median of 3 — variance taming »** : documente les deux jobs et la rationale variance.

### Suite audit perf — CLS desktop, TBT mobile, housekeeping (post-blindage)

Suite de la passe perf entamée dans la session précédente (`b7699d1`, `d972cb3`, `b03f287`, `3d5adac`, `02e5e4c`). Mesure CLS faite avec Playwright/Chromium en Docker (CDP `Emulation.setCPUThrottlingRate`) puisque pas de Chrome local — méthode reproductible, à brancher dans `lighthouse/` plus tard si besoin.

- **`docs/assets/css/style.css` CLS — margin du placeholder `#ats-search` (CLS desktop manifeste/faq)** : `search.js` ajoute la classe `.ats-search` (qui apporte `margin: 0.75rem 0 1.5rem`) à `<div id="ats-search">` une fois booté, ce qui déplaçait `<article id="content">` de **+34 px** vers le bas. La règle CSS du placeholder réserve maintenant la même margin que la classe injectée — CLS sur `fr/manifeste.html` mesuré **0.0280 → 0.0009** (-97 %) avec throttling CPU 4×. Pages `faq.{fr,en}` / `manifesto.en.html` bénéficient du même fix (même placeholder, même `search.js`).
- **`docs/assets/js/clock-page.js` lazy-init de la face analogique (TBT mobile `/fr/index`)** : `buildAnalogTicks()` (110 SVG elements) + `buildCityArcs()` (≈80 SVG elements) étaient appelés au boot même quand la face analog était `hidden`. Refacto derrière `ensureAnalogBuilt()` (flag idempotent) appelé uniquement quand `selectFace('analog')` est invoqué. `rebuildCityList()` (add/remove de villes perso) skip `buildCityArcs` tant que `analogBuilt === false` — le prochain build prend les `CITIES` fraîches. Aucune régression sur le tab switch (deep-link `?face=analog` continue de bâtir avant la première frame visible). **Gain TBT estimé ~200-300 ms** sur Moto G4 emulation.
- **`lighthouse/ci-assert.mjs` + `lighthouse/README.md` cleanup skip `uses-text-compression`** : le skip avait été ajouté en `3d5adac` mais c'est une « opportunity » Lighthouse (pas un metric noté) — zéro impact sur le score Performance. Retiré de `LOCALHOST_SKIP_AUDITS` et de la doc. Le skip `is-on-https` reste (impact réel sur BP weight 5/27).
- **`pyproject.toml` filterwarnings convertdate `month_days` deprecation** : 281 `DeprecationWarning` à chaque collection pytest, émis par convertdate (interne — `code/bridges/hebrew.py` n'appelle pas `month_days` directement, vérifié par grep). `[tool.pytest.ini_options]` filtre maintenant ces warnings ciblement (`module="convertdate"`). À re-évaluer quand `convertdate>=` est bumpé.
- **`lighthouse/archive/` housekeeping baselines v0.7-blindage** : les 3 fichiers `v0.7-blindage-{embed-en,en_code-html,en_timeline-html}.json` (~1.9 MB cumul, hors set CI des 4 pages) déplacés dans `lighthouse/archive/` avec README expliquant leur rôle. La racine `lighthouse/` ne contient plus que les artefacts vivants.
